...
For `.txt` reports, use plain text only:

```text
一、已完成事项

优化 dag-viewer 本地调试体验，明确其定位是给开发者自用的调试台而非外部平台，补齐执行记录、节点状态、时间线、节点检查器和观测摘要等能力，并完成基础验证。
新增菜单推荐能力，由大模型直接生成餐饮推荐结果，不依赖外部召回流程；失败时如实暴露原因、不做本地兜底，并完成成功与失败链路验证。
...
应用链路通畅不等于业务结果可用，模型选型与提示词质量仍需在真实环境验证。
```

Do not include Markdown syntax in `.txt` reports: no `#` headings, no `-` bullets, no backticks, no fenced code blocks, and no Markdown links. For `.md` reports, Markdown headings are allowed but the content should still read like normal prose.

## Style

- Write in Chinese.
- Keep each individual completed-work item in `一、已完成事项` around 130-150 Chinese characters, including punctuation, unless the user explicitly asks for a detailed version. The full report may be longer when there are multiple items.
- Prefer plain human wording over technical replay.
- Do not present reading, studying, or "梳理" as standalone completed work. If reading informed a concrete outcome, fold it into the delivered result, such as "明确调试台定位并完成体验优化".
...
```text
今天主要完成 dag-viewer 调试台优化、菜单推荐能力开发、服务目录规范化和 AI 工具链更新，并完成关键路径验证。
```
...
Target 3-5 work streams when the day's material is rich. If there are more than five raw items, combine them by module or goal. Keep each completed-work item around 130-150 Chinese characters; do not compress the entire section to 130 characters.
...
完成服务目录规范化，统一目录名与部署模块名，同步更新入口、构建、部署、测试和文档映射，并确认旧路径与旧引用已清理。
```
