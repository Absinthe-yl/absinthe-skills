...
- Use one topic for one business/workstream unless there is a clearly different owner-facing goal or acceptance path. For example, merge Langfuse reliability, field completion, and test-environment validation into one `Langfuse 链路接入与验收` topic; merge situation-awareness-10s migration, replay validation, MMData, and DAG Viewer support into one `situation-awareness-10s 迁移验收` topic when they all support the same migration; merge gift data debugging, tuning-table support, and MCP gift interfaces into one `礼物数据与接口能力` topic when they all support gift-data delivery.
